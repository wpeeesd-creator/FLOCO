    ...
    getDoc(doc(db, 'users', user.id)).then((snap) => {
      if (snap.exists()) {
        ...
        if (school) {
          ...
          setCohort(school.cohort ?? school.grade ?? '');
        }
      }
    }).catch(() => {});
  }, [user?.id]);

  const isReady = schoolName.trim() && cohort;

  const saveSchoolInfo = async () => {
    if (!isReady || !user?.id) {
      Alert.alert('알림', '학교명과 기수를 모두 입력해주세요');
      return;
    }
    ...
    try {
      await updateDoc(doc(db, 'users', user.id), {
        school: {
          ...
          cohort,
          classId: `${schoolName.trim()}_${cohort}`,
        },
      });
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert(
        '설정 완료!',
        `${schoolName} ${cohort} 친구들과 랭킹 경쟁을 시작해요!`,
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.title}>같은 기수 친구들과 경쟁해요!</Text>
